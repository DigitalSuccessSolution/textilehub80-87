        ...
        <div className="fixed bottom-6 right-6 z-[100] font-sans">
            ...
            {!isOpen && (
                <button
                    ...
                    className="w-14 h-14 bg-[#9C4B6E] hover:bg-[#803856] text-white rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 border border-[#C29E6B]/30 hover:scale-105 active:scale-95 cursor-pointer"
                >
            ...
            {/* Chat Window */}
            {isOpen && (
                <div className="bg-[#2E081B] border border-white/10 shadow-2xl w-80 sm:w-96 h-[480px] rounded-3xl overflow-hidden flex flex-col">
                    {/* Header */}
                    ...
                    {/* Messages Body */}
                    <div className="flex-1 p-4 overflow-y-auto space-y-4 no-scrollbar bg-[#2E081B]">
                        {messages.map((msg) => (
                            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div
                                    className={`max-w-[75%] p-3 text-xs leading-relaxed text-left ${msg.sender === 'user'
                                        ? 'bg-[#C29E6B] text-[#2E081B] rounded-2xl rounded-tr-none font-bold'
                                        : 'bg-[#3E0F26]/80 text-gray-200 border border-white/5 rounded-2xl rounded-tl-none font-light'
                                        }`}
                                 >
                                    <p>{msg.text}</p>
                    ...
                    <div className="px-4 py-2 bg-[#081E27] border-t border-white/5 flex gap-2 overflow-x-auto no-scrollbar">
                        <button
                            ...
                            className="shrink-0 px-3 py-1 text-[8px] font-bold tracking-wider border border-[#C29E6B]/30 text-[#C29E6B] rounded-full hover:bg-[#C29E6B] hover:text-[#2E081B] transition-colors cursor-pointer"
                        >
                            LUXURY SAREES
                        </button>
                        <button
                    ...
                    <form onSubmit={handleSend} className="p-3 bg-[#081E27] border-t border-white/5 flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-[#2E081B] border border-white/5 rounded-full px-4 py-2 text-xs text-white focus:outline-none focus:border-[#C29E6B] transition-colors"
                        />
                        <button
                            type="submit"
                            className="w-8 h-8 bg-[#C29E6B] text-[#2E081B] rounded-full flex items-center justify-center hover:bg-white transition-colors shrink-0 cursor-pointer"
                        >
                            <Send size={14} />
                        </button>
                    </form>
