...
const featuredPost = {
  ...
  readTime: '8 min read',
  author: 'Aarav Patel',
  image: 'https://images.pexels.com/photos/34692229/pexels-photo-34692229.jpeg'
};
...
const allPosts = [
  {
    ...
    desc: 'Learn how B2B buyers can maximize profit margins using our live bidding floor.'
  },
  {
    id: 3,
    ...
    desc: 'Exploring the eco-friendly cultivation and wholesale demand for long-staple cotton.'
  },
  {
    id: 4,
    ...
    desc: 'An essential compliance update for wholesale buyers and textile manufacturers.'
  },
...
const allCategories = ['All', ...Array.from(new Set([featuredPost.category, ...allPosts.map(p => p.category)]))];
    ...
    <div className="bg-background min-h-screen">
      ...
      <section className="py-20 border-b border-slate-200 text-center">
        <div className="container-custom max-w-4xl">
          <h1 className="font-heading text-4xl md:text-6xl font-bold text-primary mb-6">The Journal</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {allCategories.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-bold tracking-wider transition-all duration-200 border ${activeCategory === cat
      ...
      <section className="container-custom pb-24">
        ...
        {filteredPosts.length === 0 ? (
          <div className="text-center py-20 text-slate-400">
            <p className="text-lg font-light">No articles in this category yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
            {filteredPosts.map((post) => (
              <Link to={`/blog/${post.id}`} key={post.id} className="group flex flex-col md:flex-row gap-6 items-start border-b border-slate-100 pb-8 hover:bg-slate-50 transition-colors p-4 -mx-4 rounded-xl">
                ...
                <div className="flex-1 space-y-2">
                  <span className="text-accent text-[10px] font-bold tracking-widest block">
                    {post.category}
                  </span>
                  <h4 className="font-heading text-xl font-bold text-primary leading-snug group-hover:text-accent transition-colors">
                    {post.title}
                  </h4>
                  <p className="text-slate-500 font-light text-xs line-clamp-2">{post.desc}</p>
                  <div className="flex items-center gap-4 text-[10px] font-semibold text-slate-400 pt-2">
