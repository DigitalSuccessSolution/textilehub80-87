    ...
    <div className="bg-background min-h-screen">
      ...
      <div className="container-custom max-w-5xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-border pb-8 mb-12">
          <div>
            <span className="text-accent font-bold capitalize tracking-[0.2em] text-xs mb-3 flex items-center gap-2">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <button className="bg-white border border-border text-primary px-4 py-2 rounded-full text-xs font-bold capitalize tracking-widest flex items-center gap-2 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-border overflow-hidden">
          {/* List Header */}
          <div className="bg-muted grid grid-cols-12 gap-4 p-6 border-b border-border text-xs font-bold capitalize tracking-widest text-muted-foreground hidden md:grid">
          ...
          <div className="divide-y divide-border">
            {circulars.map((circ, idx) => (
              <div key={idx} className="group grid grid-cols-1 md:grid-cols-12 gap-4 p-6 items-center hover:bg-muted/50 transition-colors">
                ...
                <div className="md:col-span-3 flex items-center gap-2 text-sm text-muted-foreground font-medium">
                  <Calendar className="w-4 h-4 text-accent md:hidden" />
                  {circ.date}
                </div>
                
                <div className="md:col-span-6">
                  <p className="font-bold text-primary text-base group-hover:text-accent transition-colors capitalize">
                    {circ.title}
                  </p>
                </div>
                
                <div className="md:col-span-3 flex justify-start md:justify-end mt-4 md:mt-0 gap-2">
                  <button className="flex items-center justify-center text-primary bg-white border border-border px-4 py-2 rounded-full hover:border-accent hover:text-accent shadow-sm transition-all text-xs font-bold capitalize tracking-widest gap-2" title="View PDF">
                    <Eye className="w-4 h-4" /> View PDF
                  </button>
                  <button className="flex items-center justify-center text-primary bg-white border border-border px-4 py-2 rounded-full hover:border-accent hover:text-accent shadow-sm transition-all text-xs font-bold capitalize tracking-widest gap-2" title="Print PDF">
          ...
          <div className="bg-white p-6 border-t border-border flex justify-between items-center text-xs font-bold capitalize tracking-widest text-muted-foreground">
