...
const MediaGallery = () => {
  ...
  const mediaItems = [
    ...
    { id: 2, type: 'image', title: 'Annual Retailers Meet 2023', img: 'https://images.unsplash.com/photo-1511556532299-8f662fc26c06?q=80&w=800&auto=format&fit=crop', desc: 'Texmart Annual Summit in Surat.' },
      ...
      <div className="container-custom pb-20">
        <div className="flex items-center justify-center gap-4 mb-12">
          ...
          <button 
            ...
            className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all flex items-center gap-2 ${filter === 'video' ? 'bg-primary text-white shadow-md' : 'bg-white text-muted-foreground hover:bg-muted border border-border'}`}
          >
            <Video className="w-3.5 h-3.5" /> Videos
          </button>
          <button 
            onClick={() => setFilter('image')}
            className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all flex items-center gap-2 ${filter === 'image' ? 'bg-primary text-white shadow-md' : 'bg-white text-muted-foreground hover:bg-muted border border-border'}`}
          >
